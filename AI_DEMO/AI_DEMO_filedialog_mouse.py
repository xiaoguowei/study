...
class App:
	...
	# ===== 以上這個區塊主要是設定臉部辨識的所需匯入的檔案 ===== #
        ...
        parser.add_argument(
            '--model', help='File path of .tflite file.', default = './classification_model/model.tflite')
        parser.add_argument(
            '--labels', help='File path of labels file.', default = './classification_model/labels.txt')
        ...
        # self.canvas_face.bind("<Enter>",self.print_mouse_status) # 滑鼠進入小部件事件
        # self.canvas_face.bind("<Leave>",self.print_mouse_status) # 滑鼠離開小部件事件
        # self.canvas_face.bind("<B1-Motion>",self.print_mouse_status) # 滑鼠離開小部件事件
        self.canvas_face.bind("<Button-1>",self.print_mouse_status) # 滑鼠離開小部件事件
        ...
        self.face_button.place(x=50, y=350, width=160, height=40)

        # 設定臉部訓練按鈕的樣式與位置
        self.fontStyle = tkFont.Font(size=16)
        self.face_button = tkinter.Button(window, text="臉部識別", font=self.fontStyle, command=self.add_face_name_list)
        self.face_button.place(x=50, y=410, width=160, height=40)
        
        # 設定影像識別按鈕的樣式與位置
        self.fontStyle = tkFont.Font(size=16)
        self.before_class_button = tkinter.Button(window, text="影像識別", font=self.fontStyle, command=self.image_class)
        self.before_class_button.place(x=50, y=470, width=160, height=40)

        # 設定影像識別模型選擇按鈕的樣式與位置
        self.fontStyle = tkFont.Font(size=16)
        self.before_class_button = tkinter.Button(window, text="選擇模型", font=self.fontStyle, command=self.model_path_dialog)
        self.before_class_button.place(x=50, y=530, width=160, height=40)
    ...
    def dir_file_error_hint_Window(self):
        self.hint = tkinter.Toplevel(self.window)
        self.hint.geometry('400x200')
        self.hint.resizable(False, False)
        ...
        self.hint_text = '檔案不存在，請檢查該路徑是否存在 model.tflite 與 labels.txt 兩個檔案。'
        self.hint_info.insert('insert',self.hint_text)
    ...
    def model_path_dialog(self):
        # path_file = filedialog.askopenfilename()
        # print (path)
        path_dir = filedialog.askdirectory()
        labels_dir_file = path_dir + '/labels.txt'
        model_dir_file = path_dir + '/model.tflite'

        print(labels_dir_file)
        print(model_dir_file)

        if not(os.path.isfile(labels_dir_file) and os.path.isfile(model_dir_file)):
            self.dir_file_error_hint_Window()
            
        self.labels = self.load_labels(labels_dir_file)
        self.interpreter = Interpreter(model_dir_file)
