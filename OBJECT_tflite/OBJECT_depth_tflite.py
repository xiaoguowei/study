...
def main():
  ...
  cv2.createTrackbar('Sample Number', 'Object Detecting....', 1, 1000, nothing)
  cv2.setTrackbarPos('Sample Number', 'Object Detecting....', 500)
  ...
  while (key_detect==0):
    ...
    Sample_num = cv2.getTrackbarPos('Sample Number','Object Detecting....')
    ...
    if (times==1):
      ...
      for num in range(len(results)) :
        ...
        point_distance_list=[]
        for point_num in range(Sample_num):
      ...
      FPS = str(np.round(1/((time.time() - start_time)),3))
    ...
    cv2.putText(color_image,
      'FPS=' + FPS, 
      (int(CAMERA_WIDTH*0.7),int(CAMERA_HEIGHT*0.085)),
    ...
    show_img = cv2.resize(color_image,
      (int(CAMERA_WIDTH*1),int(CAMERA_HEIGHT*1)))
