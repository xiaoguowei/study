  ...
  while (key_detect==0):
    ...
    if (times==1):
      ...
      for num in range(len(results)) :
        ...
        cv2.rectangle(image_src,(box_left,box_top),(box_right,box_bottom),(255,255,0),3)
        cv2.putText(image_src,labels[label_id] +' score=' +str(round(results[num]['score'],3)),(box_left,box_top+20),cv2.FONT_HERSHEY_SIMPLEX,0.6,(0,255,255),2,cv2.LINE_AA)

        print(results[num],labels[label_id])
        print(box_left,box_top,box_right,box_bottom)
        print("***************************************************************")

      show_img = cv2.resize(image_src,(int(CAMERA_WIDTH/2),int(CAMERA_HEIGHT/2)))
